...
import tiktoken
from ocr import ImageTextExtractor  # Import de la classe pour l'extraction
...
class TokenBasedSentenceSplitter(TextSplitter):
    def __init__(self, chunk_size=800, chunk_overlap=50, model=MODEL):
        ...
        self.model = model
        self.image_text_extractor = ImageTextExtractor()  # Instance d'ImageTextExtractor
        ...
        for doc in documents:
            ...
            for idx, image_stream in enumerate(images):
                if isinstance(image_stream, BytesIO):
                    ...
                    image_text += " " + extracted_text  # Concatène le texte extrait
